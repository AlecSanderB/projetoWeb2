...
export default function MachineHistoryPanel({ machineId, darkMode }) {
  ...
  const xAxisInterval = useMemo(() => chartData.length <= 10 ? 0 : Math.ceil(chartData.length / 10), [chartData]);
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
        ...
        <div style={{ display: "flex", gap: "15px", alignItems: "center" }}>
          <div style={toggleButtonStyle(showRendered)} onClick={() => setShowRendered(!showRendered)}>
            <span style={{
              display: "inline-block", width: "12px", height: "12px", borderRadius: "50%", backgroundColor: "#f0dd05", marginRight: "5px"
            }} />
            Rendered
          </div>
          <div style={toggleButtonStyle(showEnabled)} onClick={() => setShowEnabled(!showEnabled)}>
            <span style={{
              display: "inline-block", width: "12px", height: "12px", borderRadius: "50%", backgroundColor: "#4caf50", marginRight: "5px"
            }} />
            Enabled
          </div>
        </div>
      ...
      <div style={{ display: "flex", marginBottom: "10px", gap: "5px", width: "100%" }}>
      ...
      {chartData.length === 0 ? (
        ...
        <ResponsiveContainer width="100%" height={200}>
          <LineChart data={chartData} margin={{ bottom: 20 }}>
            ...
            <XAxis dataKey="timestamp" stroke={darkMode ? "#fff" : "#000"} interval={xAxisInterval} angle={-30} textAnchor="end" height={40} />
